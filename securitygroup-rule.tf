...
SG_IDS=$(aws ec2 describe-security-groups \
  --query 'SecurityGroups[?!contains(GroupName, `alb`) && !contains(GroupName, `nlb`) && !contains(GroupName, `eks-cluster`) && !contains(GroupName, `efs`) && !contains(GroupName, `elasticache`) && !contains(GroupName, `jennifer`)].GroupId' \
  --output text)

for SG_ID in $SG_IDS; do
  echo "Adding rules to Security Group: $SG_ID"
  aws ec2 authorize-security-group-egress --group-id $SG_ID --ip-permissions '[
    {"IpProtocol":"tcp","FromPort":443,"ToPort":443,"UserIdGroupPairs":[{"GroupId":"sg-0d2c0e373ba88e129", "Description":"VPC Endpoint-interface"}]},
...
for SG_ID in $SG_IDS; do
  ...
  ]' 2>/dev/null || echo "Ingress rules may not exist for $SG_ID"
done
...
for SG_ID in $SG_IDS; do
  ...
  aws ec2 revoke-security-group-egress --group-id $SG_ID --ip-permissions '[
    {"IpProtocol":"tcp","FromPort":443,"ToPort":443,"UserIdGroupPairs":[{"GroupId":"sg-0d2c0e373ba88e129"}]},
...
for SG_ID in $SG_IDS; do
  echo "Adding rules to Security Group: $SG_ID"
  aws ec2 authorize-security-group-ingress --group-id $SG_ID --ip-permissions '[
    {"IpProtocol":"tcp","FromPort":22,"ToPort":22,"IpRanges":[{"CidrIp":"10.221.32.6/32","Description":"Prd workbench server"}]},
...
for SG_ID in $SG_IDS; do
  echo "Removing rules from Security Group: $SG_ID"
  aws ec2 revoke-security-group-ingress --group-id $SG_ID --ip-permissions '[
    {"IpProtocol":"tcp","FromPort":22,"ToPort":22,"IpRanges":[{"CidrIp":"10.221.32.6/32"}]},
    {"IpProtocol":"tcp","FromPort":22,"ToPort":22,"PrefixListIds":[{"PrefixListId":"pl-05f5d1bfbb8cde8f0"}]}
  ]' 2>/dev/null || echo "Ingress rules already removed or not present for $SG_ID"
done
